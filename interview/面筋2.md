...
## 几个关于js的函数实现
for .. of适合数组  for .. in 适合对象   
ES6 对数组的遍历操作：reduce、some、every、map
1. 将字符串的应为大小写互换
```javaScript
function toChangeChar(str){
    return str.replace(/\w/g,function(item){
      if(item.charCodeAt() < "Z".charCodeAt() && item.charCodeAt() > "A".charCodeAt()){
...
//判断是否为数组以及长度
  var map = {};
  var maxHistoryTime = 1;
  var maxTime = 1;
  for(var i = 1;i<arr.length;i++){  //注意如果for循环直接引用len则不能改变arr数组
    if(arr[i] - arr[i-1] == 0){
        maxTime ++;
    }else{
        map[arr[i-1]] = maxTime;
        if(maxHistoryTime < maxTime){
          maxHistoryTime = maxTime;
        }
        maxTime = 1;
    }
  }
  ...
  return "重复最多的数字是："+maxKey+",重复次数为："+max;
}
```
4. 

## CSS
渐变：linear-gradient(270deg,#3C6CDE 0,#1FA4FF 100%)
...
## 前端实现上传文件，后端返回文件流  
 ...
 //上传
 ---h5
 <a-upload name = "file" accept=".csv, application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" @change='upload'>
  <a-butoon><a-icon  type = "upload"></a-button>
 <a-upload>
 
 ---js
 upload(n){
    var form = new FormData()
    form.append('file',n.file);
    $.ajax({
      type:post
      data:form
    })
...
function export(data){
    ...
    let name = data.header["content-disposition"] || ""; //返回的文件流带有该响应头 ，设置name为了输出文件名
    if(decode){
        let xls = name.split(".")[1] //xlsx
        name = base64.decode(name.split(".")[0]) + `.${ext}`
    }
    if("msSaveOrOpenBlob" in navigator){
      return navigator.msSaveOrOpenBlob(blob,name);
    }
    let link = document.createElememt("a");
    link.href = URL.createobjectURL(blob);
    //style.display:none 
    link.download = name;
    link.click();//click事件要触发浏览器才能下载
    //设置超时为了不要点击后就去掉了a链接，URL.revokeObjectURL(blob); removeChild(link);
}
```
